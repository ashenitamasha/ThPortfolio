...
const SingleProject = ({ name, year, align, image,}) => {
  return (
    ...
    >
      <div>
        <h2 className="md:text-2xl sm:text-2xl text-cyan ">{name}</h2>
        <h2
      ...
      <div className="max-h-[220px] max-w-[400px] rounded-xl overflow-hidden hover:scale-110 transform transition-all duration-500 ralative border border-white">
        <div className="w-full h-full bg-orange opacity-25 absolute top-0 left-0 hover:opacity-0 transition-all duration-500 md:block sm:hidden"></div>
